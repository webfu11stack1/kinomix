...
with sqlite3.connect('kinomix.db') as conn:
    cursor = conn.cursor()
...
TOKEN = "8068158858:AAEX7UucJQl88uer2tLRcBQelA9nCg1DSco"
bot = Bot(token=TOKEN)
...
async def panel(message: types.Message, state: FSMContext):
    ...
    if mes_id in admin_user_ids or mes_id == 852018668:
                ...
                InlineQueryResultArticle(
                    ...
                    reply_markup=InlineKeyboardMarkup().add(
                        InlineKeyboardButton(
                            "📽 Kinoni ko'rish",
                            url=f"https://t.me/KINOMIX_REKBOT?start={result['movie_code']}"
                        )
                    )
                )
            )
    ...
    async def forward_to_user(user_id):
        ...
        try:
            await bot.forward_message(
                chat_id=user_id,
                from_chat_id=-1003413126526,  # 🎯 Kanal ID (bot admin bo‘lishi shart)
                message_id=msg_id
            )
            yetkazilgan += 1
        except BotBlocked:
...
async def export_db_command(message: types.Message, state: FSMContext):
    ...
    db_file_path = 'kinomix.db'  # Bazangizning yo'li
    ...
    with sqlite3.connect('kinomix.db') as conn:
        ...
        # )
            ...
            if unsubscribed_channels:
                ...
                keyboard.add(InlineKeyboardButton(text="➕ Obuna bo'lish 1", url="https://www.instagram.com/kinomix.rek"))
                ...
                keyboard.add(InlineKeyboardButton(text="Tekshirish ✅", url="https://t.me/KINOMIX_REKBOT?start=True"))
...
async def premium_info(callback_query: types.CallbackQuery,state:FSMContext):
    ...
    buy_button = InlineKeyboardMarkup(row_width=2).add(
        ...
        InlineKeyboardButton("⬅️ Orqaga", url="https://t.me/KINOMIX_REKBOT?start=True")
    )
...
CHANNEL_ID_PRM = -1003419527246  # admin kanal ID
    ...
    try:
        if movie_code:
            ...
            keyboard.add(
                InlineKeyboardButton(
                    text="🎥 Kino ko'rish", 
                    url=f"https://t.me/KINOMIX_REKBOT?start={movie_code}"
                )
            )
            ...
            botga = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 Botga o'tish", url="https://t.me/KINOMIX_REKBOT"),
